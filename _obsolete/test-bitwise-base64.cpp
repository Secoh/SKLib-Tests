...
public:
    base64_t(std::vector<uint8_t>& data) : data_ptr(&data), k_data(0), bits_stream_base_type(pop_byte, push_byte) {}
...
protected:
    static void push_byte(bits_stream_base_type* root, uint8_t d)
    {
        static_cast<base64_t*>(root)->data_ptr->push_back(d);
    }

    static bool pop_byte(bits_stream_base_type* root, uint8_t& d)
    {
        auto myself = static_cast<base64_t*>(root);
        if (myself->k_data >= myself->data_ptr->size()) return false;
        d = (*myself->data_ptr)[myself->k_data++];
        return true;
    }
